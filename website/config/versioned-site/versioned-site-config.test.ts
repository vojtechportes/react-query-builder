...
describe('versioned site configuration', () => {
  it('starts the v2 site from the default development command', () => {
    expect(websitePackage.scripts.dev).toBe(websitePackage.scripts['dev:v2']);
    expect(websitePackage.scripts.dev).toContain(
  ...
  it.each(['v1', 'v2'] as const)(
    ...
    (target) => {
      ...
      const stageRoot = resolve(websiteRoot, '.versioned-dist', target);
