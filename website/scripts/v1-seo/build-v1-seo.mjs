import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { createV1PageHtml } from './utils/create-v1-page-html.util.mjs';
import { createV1Robots } from './utils/create-v1-robots.util.mjs';
import { createV1Sitemap } from './utils/create-v1-sitemap.util.mjs';
import { getV1RouteOutputPath } from './utils/get-v1-route-output-path.util.mjs';
import { validateV1SeoOutput } from './utils/validate-v1-seo-output.util.mjs';

const scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
const websiteRoot = path.resolve(scriptDirectory, '../..');
const distRoot = path.join(websiteRoot, '.versioned-dist', 'v1');
const ssgRoot = path.join(distRoot, '.ssg');
const serverEntryUrl = pathToFileURL(
  path.join(ssgRoot, 'entry-server.mjs')
).href;
const { renderPage, v1RouteManifest, v1RouteRedirectManifest } = await import(
  serverEntryUrl
);
const seoConfig = JSON.parse(
  fs.readFileSync(
    path.join(
      websiteRoot,
      'src',
      'v1',
      'seo',
      'constants',
      'v1-seo-pages.json'
    ),
    'utf8'
  )
);
const configuredSiteUrl =
  process.env.VITE_CANONICAL_SITE_URL ||
  process.env.VITE_SITE_URL ||
  seoConfig.siteUrl;

seoConfig.robotsDirective =
  process.env.VITE_ROBOTS_DIRECTIVE || seoConfig.robotsDirective;
let siteUrl;

try {
  siteUrl = new URL(configuredSiteUrl).toString();
} catch {
  throw new Error(
    `VITE_CANONICAL_SITE_URL must be an absolute URL, received: ${configuredSiteUrl}`
  );
}

const pagesByPath = new Map(seoConfig.pages.map((page) => [page.path, page]));
const routesByPath = new Map(
  v1RouteManifest.map((route) => [route.path, route])
);

if (
  pagesByPath.size !== seoConfig.pages.length ||
  routesByPath.size !== v1RouteManifest.length ||
  pagesByPath.size !== routesByPath.size
) {
  throw new Error(
    'v1 SEO registry and route manifest have different route sets.'
  );
}

for (const page of seoConfig.pages) {
  const route = routesByPath.get(page.path);

  if (!route) {
    throw new Error(`v1 SEO route is not in the route manifest: ${page.path}`);
  }
  if (
    route.publicPath !==
    `${seoConfig.versionPath}${page.path === '/' ? '' : page.path}`
  ) {
    throw new Error(`v1 route has an incorrect public path: ${page.path}`);
  }
}

const indexPath = path.join(distRoot, 'index.html');
const baseHtml = fs.readFileSync(indexPath, 'utf8');

for (const route of v1RouteManifest) {
  const page = pagesByPath.get(route.path);
  const outputPath = getV1RouteOutputPath(distRoot, route.path);
  const html = createV1PageHtml({
    baseHtml,
    page,
    route,
    renderedPage: renderPage(route.path),
    seoConfig,
    siteUrl,
  });

  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, html);
}

fs.writeFileSync(
  path.join(distRoot, 'sitemap.xml'),
  createV1Sitemap(v1RouteManifest, seoConfig, siteUrl)
);
fs.writeFileSync(
  path.join(distRoot, 'robots.txt'),
  createV1Robots(seoConfig, siteUrl)
);
fs.writeFileSync(
  path.join(distRoot, 'route-redirect-manifest.json'),
  `${JSON.stringify(v1RouteRedirectManifest, null, 2)}\n`
);

const errors = validateV1SeoOutput({
  distRoot,
  pages: seoConfig.pages,
  seoConfig,
  siteUrl,
});

if (errors.length > 0) {
  throw new Error(`v1 SEO build validation failed:\n- ${errors.join('\n- ')}`);
}

fs.rmSync(ssgRoot, { recursive: true, force: true });
