...
const seoConfig = JSON.parse(
  fs.readFileSync(
    path.join(websiteRoot, 'src', 'constants', 'seo-pages.json'),
    'utf8'
  )
);
const appSource = fs.readFileSync(
  path.join(websiteRoot, 'src', 'app', 'app-routes.tsx'),
  'utf8'
);
const searchSource = fs.readFileSync(
  path.join(websiteRoot, 'src', 'hooks', 'use-site-search.ts'),
  'utf8'
);
const sitemapSource = fs.readFileSync(
  path.join(websiteRoot, 'public', 'sitemap.xml'),
  'utf8'
);
...
const ftpWorkflowSource = fs.readFileSync(
  path.join(repoRoot, '.github', 'workflows', 'deploy-website-ftp.yml'),
  'utf8'
);
...
if (fs.existsSync(path.join(websiteRoot, 'public', '.htaccess'))) {
  warn('FTP .htaccess must not be present in website/public.');
}

if (
  !ftpWorkflowSource.includes(
    'cp .github/deploy/ftp/.htaccess website/dist/.htaccess'
  )
) {
  warn(
    'FTP workflow must copy the hosting configuration after the website build.'
  );
}
...
if (
  !websitePackage.scripts['recipes:validate']?.includes('tsc') ||
  !websitePackage.scripts.build?.includes('recipes:validate')
) {
  warn('The website build must type-check published recipe snippets.');
}
...
const distRoot = target
  ? path.join(websiteRoot, '.versioned-dist', target)
