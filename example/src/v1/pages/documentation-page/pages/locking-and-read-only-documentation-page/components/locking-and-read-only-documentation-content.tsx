...
import { lockingSnippet } from '../constants/locking-snippet';
import { targetedReadOnlySnippet } from '../constants/targeted-read-only-snippet';
import { lockingGuiSnippet } from '../constants/locking-gui-snippet';
import { lockToggleSnippet } from '../constants/lock-toggle-snippet';
import { cloneButtonSnippet } from '../constants/clone-button-snippet';
...
export const LockingAndReadOnlyDocumentationContent: React.FC = () => (
  <>
    ...
    <p>
      ...
      query data model, so the resulting lock state is preserved in output. If a
    ...
    <List>
      ...
      <li>
        When a parent group inherits a lock to descendants, child lock controls
        render disabled because descendants cannot override that inherited
        state.
      </li>
      <li>
    ...
    <List>
      ...
      <li>
        If <InlineCode>enabled</InlineCode> is <InlineCode>true</InlineCode> and{' '}
        <InlineCode>targets</InlineCode> is omitted, the whole node is
        read-only.
      </li>
      <li>
    ...
    <List>
      ...
      <li>
        Targeted rule read-only also blocks deleting that rule, even if only one
        target such as <InlineCode>field</InlineCode> is protected.
      </li>
      <li>
        Locked groups cannot change their group operator, negation, add actions,
        or delete action.
      </li>
      <li>
    ...
    <List>
      ...
      <li>
        Group target inheritance preserves only the group-level semantics of the
        inherited lock. It does not turn rule-specific controls such as{' '}
