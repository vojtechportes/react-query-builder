...
const transitionModules = [
  'clone-button/clone-button.module.css',
  'drop-zone/drop-zone.module.css',
  'empty-group-drop-zone/empty-group-drop-zone.module.css',
  'fluentui/shared/components/fluent-ui-group-header-option/fluent-ui-group-header-option.module.css',
  'form/switch/switch.module.css',
  'lock-toggle/lock-toggle.module.css',
  'widgets/select-multi/components/option/option.module.css',
];
...
describe('T059 visual and accessibility parity contract', () => {
  it('keeps desktop and responsive root, group, rule, and control layouts', () => {
    ...
    const group = readCss(
      'group/components/group-container/group-container.module.css'
    );
    const rule = readCss('rule/rule.module.css');
  ...
  it('keeps DnD, validation, clone, lock, text-mode, and Monaco state hooks', () => {
    const dropZone = readCss('drop-zone/drop-zone.module.css');
    const emptyDropZone = readCss(
      'empty-group-drop-zone/empty-group-drop-zone.module.css'
    );
    const rule = readCss('rule/rule.module.css');
    const cloneButton = readCss('clone-button/clone-button.module.css');
    const lockToggle = readCss('lock-toggle/lock-toggle.module.css');
  ...
  it('keeps visible keyboard focus and minimum control hit targets', () => {
    const button = readCss('button/button.module.css');
    const cloneButton = readCss('clone-button/clone-button.module.css');
    const lockToggle = readCss('lock-toggle/lock-toggle.module.css');
    const popoverItem = readCss('popover-item/popover-item.module.css');
    const switchCss = readCss('form/switch/switch.module.css');
