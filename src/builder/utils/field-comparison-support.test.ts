...
} from './field-comparison-support.util';
...
describe('field comparison support', () => {
  ...
  it('supports scalar and configured single-value field comparisons', () => {
    ...
    expect(supportsFieldComparisonForOperator(fields[6], 'LARGER_EQUAL')).toBe(
      true
    );
  });

  it('rejects unsupported operators and fields without semantic comparison types', () => {
    expect(supportsFieldComparisonForOperator(fields[0], 'BETWEEN')).toBe(
      false
    );
    expect(supportsFieldComparisonForOperator(fields[0], 'IN')).toBe(false);
    expect(supportsFieldComparisonForOperator(fields[0], 'IS_NULL')).toBe(
      false
    );
