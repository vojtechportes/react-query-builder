...
export const formatSpelNode = (
  node: DenormalizedNode,
  modifierlessGroupCombinator: QueryGroupValue
): string => {
  if (!isGroupNode(node)) {
    return formatSpelRule(node);
  }

  return formatSpelGroup(node, modifierlessGroupCombinator);
};

export const formatSpelGroup = (
  ...
  const inner = joinFragments(
    group.children
      .map(child => formatSpelNode(child, modifierlessGroupCombinator))
