...
const resolveChanges = (changes: ITextModelChangeLike[]): IResolvedChange[] => {
...
export const restoreSelectionsBeforeChange = (
  ...
  return selections.map((selection) => {
    ...
    const restoredStart = restoreOffsetBeforeChange(
      selectionStart,
      resolvedChanges
    );
    const restoredEnd = restoreOffsetBeforeChange(
      selectionEnd,
      resolvedChanges
    );
