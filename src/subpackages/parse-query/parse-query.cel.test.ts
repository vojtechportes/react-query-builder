...
describe('parseQuery CEL', () => {
  it('parses logical groups and comparisons', () => {
    const result = parseQuery('(status == "active" && age >= 18)', 'CEL');
  ...
  it('still collapses literal ranges normally', () => {
    const result = parseQuery('(price >= 10 && price <= 20)', 'CEL');
        ...
        children: [
          {
            ...
            operator: 'CONTAINS',
            valueSource: 'field',
            valueField: 'profile.needle',
          },
          {
            field: 'profile.name',
            operator: 'STARTS_WITH',
            valueSource: 'field',
            valueField: 'profile.prefix',
          },
          {
            field: 'profile.name',
            operator: 'ENDS_WITH',
            valueSource: 'field',
            valueField: 'profile.suffix',
          },
          {
            field: 'profile.name',
            operator: 'LIKE',
            valueSource: 'field',
            valueField: 'profile.pattern',
          },
        ],
      },
    ]);
